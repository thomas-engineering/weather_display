...
Use Phosphor icons (https://phosphoricons.com) throughout — in work that renders in a browser. The LVGL target has no icon font at all; see "Consuming target: LVGL / ESP32-P4" below before specifying an icon that also has to appear on the device.
...
## Consuming target: LVGL / ESP32-P4

One consuming project does not render in a browser: a 1024×600 MIPI-DSI panel driven by LVGL 9.5 on an ESP32-P4. `screens/weather-display.html` shows how the system currently lands there. LVGL has no cascade, no `color-mix()`, no pseudo-classes and no text engine beyond bitmap fonts, so a specification that assumes CSS silently becomes something else on the device. Design around these:

- **Type is pre-generated at fixed sizes.** The Inter faces exist as bitmaps at 10, 12, 14, 16, 18, 20, 22, 36 and 48px — caption sizes at 400, display sizes at 500. No intermediate sizes, no variable axes, no synthetic bolding: a scale step outside that list cannot be rendered. The generated range is ASCII plus the Latin-1 supplement and a few punctuation marks, so any glyph beyond that has to be asked for when the faces are built.
- **Icons are not Phosphor here.** Two sources exist on the panel: a handful of FontAwesome glyphs baked into LVGL's Montserrat builds, and shapes drawn at runtime from primitives — circles, rounded rectangles, a polyline. An icon this system specifies has to be expressible as one of those, or it does not exist on the device.
- **A symbol and its label are never the same face.** Those symbol glyphs live only in the Montserrat builds, not in the generated Inter faces, so an icon set inside a line of text changes font mid-line. Design for that rather than against it.
- **Colour is RGB565.** At 16 bits a wide gradient field bands visibly. The ramps are safe; a flood-to-flood gradient across a large surface is not.
- **Interaction states have to survive as widget styles.** `:focus-visible`, `outline-offset` and `color-mix()` have no equivalent — a focus ring is a border plus padding on a state, and every tint must resolve to a ramp step at design time rather than being mixed at render time.
- **No locale data, no hyphenation.** Dates, weekdays and clock formats are composed by the application and arrive as finished strings; LVGL neither formats them nor breaks words. Text that must not overflow needs a length budget in the design, not an ellipsis rule.
- **`.lighten` does not apply.** There are no photographs on the device.

The panel is a fixed 1024×600 surface: a layout for it is fitted to that frame, not flowed across a viewport range.
...
- `screens/weather-display.html` — how the system renders on the LVGL / ESP32-P4 target, captured from the firmware itself rather than mocked up.
...
- `assets/weather-display-sim.png` — the panel screenshot `screens/weather-display.html` shows.
