/**
 * @file lv_conf.h — LVGL-Konfiguration fuer den Host-Simulator (sim/).
 *
 * Kopie von managed_components/lvgl__lvgl/lv_conf_template.h (v9.5.0) mit genau
 * diesen Abweichungen:
 *
 *   1. #if 0 -> #if 1                          Template-Guard aufgemacht
 *   2. LV_USE_STDLIB_MALLOC = LV_STDLIB_CLIB   host-malloc; der eingebaute Pool
 ...
 *
 * LV_COLOR_DEPTH bleibt bei 16 — identisch mit CONFIG_LV_COLOR_DEPTH_16 des
 * Geraetebuilds, damit Farbverlaeufe und Dithering hier so aussehen wie dort.
 *
 * Das ist NICHT die Konfiguration der Firmware: die kommt auf dem P4 aus
 * Kconfig (managed_components/lvgl__lvgl/Kconfig -> sdkconfig). Wer dort etwas
 * umstellt, das die Darstellung beeinflusst, muss es hier nachziehen.
 */
...
/* clang-format off */
#if 1 /* enabled: this is a real lv_conf.h, not the template */
